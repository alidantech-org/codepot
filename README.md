...
Every package under `packages/python/` belongs to one root `uv` workspace. `uv` owns dependency resolution, the shared `uv.lock`, the root `.venv`, editable workspace-member installation, and command execution.

Install `uv` once, then run from the repository root:
...
There is no package-by-package virtual-environment setup or activation step. Do not use `python -m venv`, `virtualenv`, `pip install -e`, or `PYTHONPATH` to connect repository packages. See [`.docs/project/python-workspace.md`](.docs/project/python-workspace.md).
