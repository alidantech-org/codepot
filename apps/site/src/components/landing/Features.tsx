...
export function Features({ features }: { features: Feature[] }) {
  return (
    <section
      id="features"
      className="relative mx-4 my-14 max-w-7xl overflow-hidden rounded-[1.5rem_1rem_2.5rem_1rem] border border-border bg-card px-5 py-8 shadow-lg shadow-primary/5 sm:mx-6 sm:my-16 sm:rounded-[2.25rem_1.5rem_3.5rem_1.5rem] sm:px-8 sm:py-10 lg:mx-auto lg:my-20 lg:px-12 lg:py-14"
    >
      ...
      <div className="relative z-10">
        ...
        <ul className="mt-6 grid grid-cols-1 gap-2 sm:mt-8 md:grid-cols-2 md:gap-x-6">
          {features.map(({ label, color }) => (
            <li
              key={label}
              className="flex items-start gap-3 rounded-xl border border-border/70 bg-background/55 px-3.5 py-3 text-[13px] leading-5 text-foreground sm:px-4 sm:py-3.5 sm:text-[14px] sm:leading-6"
            >
              <Dot color={color} />
