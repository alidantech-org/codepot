...
Validation: strict source and test typecheck passed before Tasks 18-20. Contract validation is now part of the combined Tasks 17-20 behavioral, declaration, architecture, build, and package gate.
...
- [x] Introduce one typed package-version source at `src/internal/package-info.ts`.
- [x] Complete producer adoption in authoring, template-pack, template-variable, generation-plan, rendered-generation, and generation-manifest assembly through Tasks 18-20 without changing serialized values.
...
- [ ] Preserve readonly and JSON-safe artifact contracts against the combined baseline suite.
...
- [x] Existing root and `codepotx/contract` consumer type fixtures compile unchanged in the first strict gate.
- [ ] Artifact JSON snapshots match the baseline after Tasks 18-20.
...
## Producer metadata completion

- Task 18 uses `CODEPOT_ARTIFACT_PRODUCER` in authoring artifact assembly.
- Task 19 uses it in template-pack and template-variable artifact assembly.
- Task 20 uses it in generation-plan, rendered-generation, and manifest assembly.
- Producer name and version remain `codepotx` and `0.0.0`; no protocol or serialized value changed.
