"""Profile CodepotG stages against a real OpenAPI document and template pack.

The bundled ``debug`` pack is synthetic and is useful only for engine diagnostics. A
release-performance claim must pass ``--templates`` with a realistic pack and should
use ``--workspace`` so generated files remain available for inspection.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Persist cache and generated output beneath this directory. Without it, "
            "the profiler uses and deletes a temporary workspace."
        ),
    )
    parser.add_argument(
        "--emit",
    ...
    if args.templates is None and args.language == "debug":
        print(
            "warning: the bundled debug pack is synthetic; pass --templates with a "
            "realistic pack before treating this run as generation evidence",
            file=sys.stderr,
        )
    ...
    if args.workspace is not None:
        ...
        return _profile(args, source=source, root=root, persistent=True)

    with tempfile.TemporaryDirectory(prefix="codepotg-memory-") as directory:
        return _profile(args, source=source, root=Path(directory), persistent=False)
...
def _profile(
    ...
    *,
    ...
    try:
        ...
        print(f"template pack: {template_root}")
        print(f"workspace: {root} ({'persistent' if persistent else 'temporary'})")
        print(
            "normalized roots: "
