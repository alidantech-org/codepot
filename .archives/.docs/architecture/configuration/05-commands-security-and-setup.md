...
Template packs may need to prepare, polish, or validate their output without forcing project users to understand internal templates. Dryv therefore supports visible pack-owned actions and commands, project-owned commands, typed setup questions, and manual instructions.
...
Declared at `commands.before` and `commands.after` in `dryv.yaml`. They run once around the selected pack sequence and are owned by the project.
...
Declared in `DryvPack.yaml`. They are authored by the pack publisher and are governed by downloaded-pack trust policy.
...
Default visible values are limited to controlled platform values such as `PATH` and Dryv-defined paths. Secrets require an explicit host-provided secret reference and must never be serialized to project files, lock files, diagnostics, or generated setup reports.
...
`dryv configure` executes the setup conversation through typed application operations. CLI, MCP, and web frontends can render the same questions.
