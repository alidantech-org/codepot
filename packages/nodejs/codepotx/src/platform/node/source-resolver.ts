...
import type { DefaultSourceResolverOptions } from '../shared/source-resolver.types';
...
export class DefaultSourceResolver implements SourceResolverPort {
      ...
      case 'memory': {
        ...
        if (!resolved) {
          throw new PlatformOperationError(
            ...
            `Unknown memory source: ${source.id}`,
          );
        }
  ...
  async #resolvePackage(
    ...
    try {
      ...
      throw new PlatformOperationError(
        ...
        `Unable to resolve package: ${source.package}`,
        { cause: caught },
      );
    }
    const packageRoot = await this.#findPackageRoot(resolvedEntry, source.package);
    const manifest = this.#codec.parseJson<{
  ...
  async #resolveGit(
    ...
    if (!await this.#files.exists(cloneRoot) || options.cache === 'bypass') {
      ...
      await this.#runGit(
        `git clone --no-checkout ${quote(source.repository)} ${quote(target)}`,
        dirname(target),
        options,
      );
      if (source.ref) {
        await this.#runGit(
          `git -C ${quote(target)} fetch --depth 1 origin ${quote(source.ref)}`,
          target,
          options,
        );
        await this.#runGit(
          `git -C ${quote(target)} checkout --detach FETCH_HEAD`,
          target,
          options,
        );
      } else {
        await this.#runGit(
          `git -C ${quote(target)} checkout --detach`,
          target,
          options,
        );
      }
  ...
  async #runGit(
    ...
    if (result.exitCode !== 0) {
      if (options.signal?.aborted) {
        throw new OperationCancelledError(options.signal.reason);
      }
      throw new PlatformOperationError(
        'GIT_COMMAND_FAILED',
  ...
  async #findPackageRoot(entry: string, expectedName: string): Promise<string> {
    ...
    while (current !== root) {
      ...
      if (await this.#files.exists(manifestPath)) {
        try {
          const manifest = this.#codec.parseJson<{ readonly name?: string }>(
            await this.#files.readText(manifestPath),
          );
          if (manifest.name === expectedName) return current;
        } catch {
          // Continue walking when a nested package manifest is not selected.
        }
      }
    ...
    throw new PlatformOperationError(
      ...
      `Unable to locate package root for ${expectedName}.`,
    );
  }
  ...
  async #describe(
    ...
    if (!await this.#files.exists(root)) {
      throw new PlatformOperationError(
        'SOURCE_ROOT_MISSING',
        `Source root does not exist: ${root}`,
      );
    }
    if (!await this.#files.exists(entry)) {
      throw new PlatformOperationError(
        'SOURCE_ENTRY_MISSING',
        `Source entry does not exist: ${entry}`,
      );
    }
    ...
    const digest = await this.#hash.values(
      references.map((file) => [file.rootRelativePath ?? file.path, file.digest ?? '']),
    );
    return {
