...
function comparisonValue(content: VirtualFileContent, mode: CompareFileRequest['compareMode']): string {
...
export class ChangedAwareFileWriter implements FileWriterPort {
  ...
  async compare(request: CompareFileRequest): Promise<CompareFileResult> {
    ...
    if (!await this.#files.exists(request.path)) {
      return { exists: false, changed: true, nextDigest };
    }
  ...
  async write(request: WriteFileRequest): Promise<FileWriteOutcome> {
    ...
    if (!comparison.changed) {
      return { path: request.path, status: 'unchanged', lifecycle: request.lifecycle };
    }
  ...
  async #writeContent(path: string, content: VirtualFileContent): Promise<void> {
    if (content.encoding === 'utf8') {
      await this.#files.writeText(path, content.text);
    } else {
      await this.#files.writeBase64(path, content.data);
    }
  }
}
