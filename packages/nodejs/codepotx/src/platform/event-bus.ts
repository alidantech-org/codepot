...
/** Sequential observational event bus with isolated listener failures. */
export class SequentialEventBus implements EventBusPort {
  ...
  publish(event: CodepotEvent): Promise<void> {
    const dispatch = async (): Promise<void> => {
      ...
      const listeners = [...this.#allListeners, ...typed];
      ...
      for (const listener of listeners) {
        try {
          ...
          try {
            ...
            // Observer errors must never alter required runtime control flow.
          }
        }
      }
    };
  ...
  subscribe<TType extends CodepotEventType>(
    ...
    if (!maybeListener) {
      throw new TypeError('A typed event subscription requires a listener.');
    }

    const listeners = this.#typedListeners.get(typeOrListener) ?? new Set<CodepotEventListener>();
    const listener = maybeListener as CodepotEventListener;
    ...
    return new EventSubscription(() => {
      ...
      if (listeners.size === 0) {
        this.#typedListeners.delete(typeOrListener);
      }
    });
  }
}
