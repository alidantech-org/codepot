...
export type ExtendWithFields = Readonly<Record<string, unknown>>;
export type ExtendWithInput = ExtendWithFields | ModelRef | RefUsage<ModelRef>;

export type ProjectionFieldSelection<
  TFields extends Record<string, unknown>,
  TMap extends Record<string, true>,
> = TMap & {
  readonly [TKey in Exclude<keyof TMap, keyof TFields>]: never;
};
...
export type SchemaExtendedRefUsage<
...
export type SchemaRefWithUsageMethods<
  TRef extends ComponentRef,
  TFields extends Record<string, unknown>,
> = Omit<RefWithUsageMethods<TRef>, 'extendWith'> & {
  extendWith<TExtensionFields extends ExtendWithFields>(
...
export interface AccessRoleSource<TValue extends string = string> {
  readonly source: PropertyRef;
  readonly allow: Readonly<Record<string, true>>;
  readonly __value?: TValue;
}

export type AccessAllowSelection<
  TValue extends string,
  TAllow extends Record<string, true>,
> = TAllow & {
  readonly [TKey in Exclude<keyof TAllow, TValue>]: never;
};

export type RefWithAccessAllowMethods<
  TRef extends PropertyRef,
  TValue extends string,
