...
export function normalizeInlineNamedSchema(value: unknown): CompiledInlineSchema {
  if (isRef(value)) {
    return {
      kind: 'object',
      fields: [],
...
export function schemaUse(value: unknown): CompiledSchemaUse {
  ...
  const base: CompiledSchemaUse = isEngineRef(source)
    ? { kind: 'ref', ref: source.id, required: requiredValue, nullable }
    : {
        kind: 'inline',
  ...
  return inlineUse(
    { kind: 'array', items: base, constraints: [] },
    requiredValue,
    nullable,
  );
}
