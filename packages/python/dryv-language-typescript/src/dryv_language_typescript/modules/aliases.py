...
def alias_specifier(
    ...
    for binding in options.aliases:
        ...
        if provider_parts[: len(root_parts)] == root_parts:
            matches.append((len(root_parts), binding.alias, provider_parts[len(root_parts) :]))
    ...
    if len(matches) > 1 and matches[0][0] == matches[1][0]:
        raise ValueError("TS_MODULE_ALIAS_AMBIGUOUS: multiple aliases match the provider equally")
