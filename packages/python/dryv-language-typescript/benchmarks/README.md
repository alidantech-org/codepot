# TypeScript isolated benchmarks
...
The v1 environment must contain the old `dryv` 1.0.0 distribution. The v2 environment must contain `dryv` plus only this adapter. Do not install both Dryv generations in one interpreter. Machine-specific outputs belong under ignored `benchmark-results/`.
