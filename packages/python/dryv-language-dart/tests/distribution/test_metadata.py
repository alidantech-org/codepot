...
def test_distribution_metadata_typing_and_entry_point() -> None:
    package = distribution("dryv-language-dart")
    assert package.version == "0.1.0a1"
    assert (Path(dryv_language_dart.__file__).parent / "py.typed").is_file()
    entry = next(
...
def test_built_wheel_contents(
    ...
    assert any(name == "dryv_language_dart/py.typed" for name in names)
    assert any(name.endswith(".dist-info/entry_points.txt") for name in names)
    assert not any(name.startswith(("tests/", "benchmarks/")) for name in names)
    assert not any("dryv_language_typescript" in name for name in names)
