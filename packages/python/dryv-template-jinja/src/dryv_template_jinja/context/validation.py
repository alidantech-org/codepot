...
from dryv import diagnostics as public_diagnostics
from dryv import ir as public_ir
from dryv.ir import Group, Name, NameProjection
...
def _freeze(value: object, *, state: _FreezeState, depth: int, path: str) -> SafeValue:
    ...
    if isinstance(value, Enum):
        if not _is_approved_enum(value):
            raise ContextSafetyError(
                "JINJA_CONTEXT_UNSAFE",
                "enum values must come from an approved public Dryv contract",
                path=path,
                value_type=type(value).__name__,
            )
    ...
    for module in (public_ir, public_diagnostics):
        for name in getattr(module, "__all__", ()):
            ...
            if (
                ...
                return True
    from dryv_template_jinja.helpers.descriptors import HelperKind

    return isinstance(value, HelperKind)


def _is_helper_descriptor(value: object) -> bool:
    from dryv_template_jinja.helpers.descriptors import HelperDescriptor
