...
These runners compare neutral rendering behavior without importing Dryv v1 and v2 into one interpreter. Correctness, sandbox safety, bounded resources, and structured failures take priority over speed.
...
## Run v1 only in a Dryv 1.0.0 environment
...
python ../dryv-template-jinja/benchmarks/run_v1_baseline.py \
  > ../dryv-template-jinja/benchmark-results/v1.json
```

On Windows PowerShell, activate with `.benchmark-v1\Scripts\Activate.ps1`.

## Run v2 only in a Dryv/Jinja environment
