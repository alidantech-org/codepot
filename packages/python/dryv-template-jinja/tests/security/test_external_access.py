...
def test_template_cannot_read_a_secret_file(tmp_path: Path, monkeypatch, render) -> None:
...
def test_template_cannot_read_environment_secret(monkeypatch, render) -> None:
...
def test_template_cannot_execute_process(monkeypatch, render) -> None:
...
def test_template_cannot_open_network_socket(monkeypatch, render) -> None:
...
def test_mutable_context_objects_are_rejected_without_mutation(render) -> None:
    value = {"safe": "value"}
    before = value.copy()
...
def test_helper_registry_does_not_change_after_failure(render) -> None:
