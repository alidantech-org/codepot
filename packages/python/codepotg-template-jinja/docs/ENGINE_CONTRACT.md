...
`codepotg-template-jinja` implements only `codepotg.ports.TemplateEngine`. A render request supplies one root template ID and source, one sorted immutable context, and one sorted in-memory partial registry. A result supplies either one text value or structured diagnostics.
...
Accepted values are `None`, booleans, finite integers/floats, UTF-8 strings, tuples of accepted values, sorted tuple-pair mappings, approved public frozen CodepotG semantic dataclasses/enums, and immutable helper descriptors. Values are converted to request-local immutable snapshots before Jinja sees them.
