...
def _build_wheel(project: Path, wheelhouse: Path) -> Path:
    ...
    expected = "codepotg_template_jinja" if project == PACKAGE_ROOT else "codepotg_core"
...
def test_wheel_contents_are_isolated(tmp_path: Path) -> None:
    ...
    assert "codepotg_template_jinja/py.typed" in names
...
def test_isolated_wheels_discover_entry_point_and_render(tmp_path: Path) -> None:
    if not CORE_ROOT.exists():
        pytest.skip("sibling codepotg-v2 package is unavailable")
...
entries = entry_points(group='codepotg.template_engines')
