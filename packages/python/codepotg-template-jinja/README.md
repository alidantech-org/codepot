...
`codepotg-template-jinja` is the independently installable, sandboxed Jinja adapter for CodepotG v2. It implements the public `codepotg.ports.TemplateEngine` protocol and renders only the source, immutable prepared context, and declared in-memory partials supplied in a `RenderRequest`.
...
- immutable snapshots of safe scalars, tuples, sorted tuple-pair mappings, public CodepotG frozen IR values, and approved enums;
...
- Python entry-point discovery through `codepotg.template_engines`.
