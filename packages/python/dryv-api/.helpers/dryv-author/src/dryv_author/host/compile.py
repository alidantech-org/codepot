...
def compile_target(request: HostCompileRequest) -> HostCompileResponse:
    try:
        author = discover_author(
            request.target,
            project_root=Path(request.project_root).resolve() if request.project_root else None,
        )
        ...
        if request.representation == "json":
            ...
            content = contract_to_jsonl(result.contract).decode("utf-8")
        else:
        ...
        return HostCompileResponse(
            False,
            None,
            None,
            (
                {
                    "code": "AUTHOR_HOST_FAILED",
                    "severity": "error",
