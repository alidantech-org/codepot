...
from .manifest import PlannedArtifactMetadata, planned_artifacts
    ...
    try:
        for event in publish_events(api.events(planned.build_id), progress):
            if event.type == "artifact.begin":
                ...
                artifact = _end(event, open_items)
                ...
                raise ArtifactVerificationError(
                    "CLI_BUILD_TERMINATED",
                    f"build ended with {event.type}",
                )
        if not stream_complete:
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_STREAM_INCOMPLETE",
                "artifact stream ended without completion marker",
            )
        if open_items:
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_STREAM_INCOMPLETE",
                "artifact stream ended with unfinished artifacts",
            )
        if set(completed) != set(metadata):
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_STREAM_INCOMPLETE",
                "artifact stream did not return exactly the planned artifacts",
            )
        artifacts = tuple(
            sorted(completed.values(), key=lambda item: item.provenance.plan_index)
        )
...
def _begin(
    event: ApiEvent,
    metadata: dict[str, PlannedArtifactMetadata],
    root: Path,
    open_items: dict[str, _OpenArtifact],
    completed: dict[str, ReceivedArtifact],
) -> None:
    ...
    if expected is None or expected.path != path:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_PLAN",
            f"artifact {artifact_id!r} does not match GenerationPlan",
        )
    _verify_provenance(document, expected, include_dependencies=True)
    if artifact_id in open_items or artifact_id in completed:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_DUPLICATE",
            f"artifact {artifact_id!r} started more than once",
        )
    ...
    if declared is not None and (
        ...
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_SIZE",
            "artifact begin size must be a non-negative integer",
        )
    content_path = root / sha256(artifact_id.encode("utf-8")).hexdigest()
    handle = content_path.open("wb")
    open_items[artifact_id] = _OpenArtifact(
        artifact_id,
        path,
        content_path,
        handle,
...
def _chunk(event: ApiEvent, open_items: dict[str, _OpenArtifact]) -> None:
    ...
    if item is None:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_ORDER",
            f"chunk arrived before artifact {artifact_id!r} began",
        )
    _verify_provenance(document, item.expected, include_dependencies=False)
    offset = document.get("offset")
    if not isinstance(offset, int) or isinstance(offset, bool) or offset != item.next_offset:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_OFFSET",
            f"artifact {artifact_id!r} chunk offset is invalid",
        )
    try:
        content = base64.b64decode(
            _string(document.get("contentBase64"), "contentBase64"),
            validate=True,
        )
    except Exception as exc:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_BASE64",
            f"artifact {artifact_id!r} chunk is invalid base64",
        ) from exc
    if not content:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_CHUNK",
            f"artifact {artifact_id!r} contains an empty streamed chunk",
        )
...
def _end(
    ...
    if item is None:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_ORDER",
            f"artifact {artifact_id!r} ended without a begin event",
        )
    try:
        _verify_provenance(document, item.expected, include_dependencies=False)
        if _string(document.get("path"), "path") != item.path:
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_PLAN",
                f"artifact {artifact_id!r} end path changed during streaming",
            )
        ...
        if not isinstance(size, int) or isinstance(size, bool) or size < 0:
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_SIZE",
                f"artifact {artifact_id!r} end size is invalid",
            )
        actual_hash = f"sha256:{item.digest.hexdigest()}"  # type: ignore[attr-defined]
        if size != item.next_offset or (
            item.declared_size is not None and item.declared_size != size
        ):
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_SIZE",
                f"artifact {artifact_id!r} size does not match streamed bytes",
            )
        if content_hash != actual_hash:
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_HASH",
                f"artifact {artifact_id!r} hash does not match streamed bytes",
            )
...
def _verify_provenance(
    ...
    *,
    ...
    if _string(document.get("jobId"), "jobId") != expected.provenance.job_id:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_PROVENANCE",
            f"artifact {expected.artifact_id!r} came from an unexpected render job",
        )
    plan_index = document.get("planIndex")
    if (
        not isinstance(plan_index, int)
        or isinstance(plan_index, bool)
        or plan_index != expected.provenance.plan_index
    ):
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_PROVENANCE",
            f"artifact {expected.artifact_id!r} has an unexpected plan index",
        )
    if include_dependencies:
        ...
        if not isinstance(dependencies, list) or not all(
            ...
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_PROVENANCE",
                "artifact dependencies must be an array of strings",
            )
        if tuple(dependencies) != expected.provenance.dependencies:
            raise ArtifactVerificationError(
                "CLI_ARTIFACT_PROVENANCE",
                f"artifact {expected.artifact_id!r} dependencies do not match GenerationPlan",
            )
...
def _string(value: object, name: str) -> str:
    if not isinstance(value, str) or not value:
        raise ArtifactVerificationError(
            "CLI_ARTIFACT_PROTOCOL",
            f"artifact event {name} must be a non-empty string",
        )
    return value
