        ...
        *,
        ...
        try:
            if stripped.startswith(("{", "[")):
                ...
                value, spans = _parse_yaml(text, source)
...
def _parse_yaml(text: str, source: LoadedSource) -> tuple[object, dict[str, SourceSpan]]:
    node = yaml.compose(text, Loader=yaml.SafeLoader)
    if node is None:
        raise ParseError("OA_PARSE_EMPTY", "source document is empty")
    spans: dict[str, SourceSpan] = {}
    value = _convert_yaml_node(node, source, "", spans)
    return value, spans
    ...
    if isinstance(node, MappingNode):
        ...
        for key_node, value_node in node.value:
            ...
            if not isinstance(key, str):
                raise ParseError("OA_PARSE_NON_STRING_KEY", "mapping keys must be strings", pointer)
            if key in result:
                raise DuplicateKeyError(key, pointer)
            child_pointer = f"{pointer}/{_escape(key)}"
            result[key] = _convert_yaml_node(value_node, source, child_pointer, spans)
        return result
    if isinstance(node, SequenceNode):
        return [
            _convert_yaml_node(item, source, f"{pointer}/{index}", spans)
            for index, item in enumerate(node.value)
        ]
    if isinstance(node, ScalarNode):
        return _scalar_value(node)
    raise ParseError("OA_PARSE_NODE", f"unsupported YAML node {type(node).__name__}", pointer)
