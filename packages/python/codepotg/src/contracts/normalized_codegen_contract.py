...
from contracts.api import ApiContract, ApiEntity, ApiOperation, ApiResource, ApiSchema
...
from contracts.normalized_domains import NormalizedAccessPolicy, NormalizedDomainView
...
class NormalizedCachePolicy:
    ...
    invalidate: NormalizedCacheInvalidation = field(
        default_factory=NormalizedCacheInvalidation
    )
    source: SourceObject = field(default_factory=SourceObject)
...
class NormalizedOperationMetadata:
    ...
    sources: ContractCollection[NormalizedDataSource] = field(
...
class NormalizedResourceMetadata:
    ...
    hooks: ContractCollection[NormalizedHookDefinition] = field(
        default_factory=ContractCollection
    )
...
class NormalizedCodegenContract:
    resources: ContractCollection[NormalizedResourceMetadata] = field(
        default_factory=ContractCollection
    )
    operations: ContractCollection[NormalizedOperationMetadata] = field(
        default_factory=ContractCollection
    )
    hooks: ContractCollection[NormalizedHookDefinition] = field(
        default_factory=ContractCollection
...
def build_normalized_codegen_contract(
    ...
    """Build resource and operation x-codegen metadata after target registries exist."""

    document = _mapping(raw)
    codegen = _mapping(document.get("x-codegen"))
    resources_raw = _mapping(codegen.get("resources"))
    policy_targets = _target_aliases(domains.access.all, ReferenceKind.ACCESS)
    resource_targets = _target_aliases(api.resources, ReferenceKind.RESOURCE)
    operation_targets = _target_aliases(api.operations, ReferenceKind.OPERATION)
    ...
    resource_ui = {item.id: item.ui for item in resource_metadata}
    ...
    return NormalizedCodegenContract(
        resources=contract_collection(resource_metadata),
        operations=contract_collection(operation_metadata),
    ...
    *,
    ...
    return NormalizedResourceMetadata(
        ...
        operations=contract_collection(
            operation for operation in api.operations if operation.resource == resource.id
        ),
        schemas=contract_collection(
            schema for schema in api.schemas.all if schema.resource == resource.id
        ),
        entities=contract_collection(
            entity for entity in api.entities if entity.resource == resource.id
        ),
...
def _resource_hooks(
    resource: str,
    raw: Mapping[str, Any],
) -> tuple[NormalizedHookDefinition, ...]:
    return tuple(
        NormalizedHookDefinition(
            ...
            notes=structured_notes(
                _mapping(value).get("info", _mapping(value).get("notes"))
            ),
...
def _ui(
    ...
    *,
    source_path: str,
    inherited: NormalizedUiSettings | None = None,
    ...
    effective = (
        PresenceValue.effective(enabled.value, source_path=f"{source_path}.effective")
        if enabled.is_set
        else inherited_enabled
        if inherited_enabled is not None and inherited_enabled.is_set
        else PresenceValue.missing(source_path=f"{source_path}.effective")
    )
...
def _cache(
    ...
    *,
    ...
    operation_names = _string_sequence(
        invalidate.get("operations", invalidate.get("operationNames"))
    )
    resource_names = _string_sequence(
        invalidate.get("resources", invalidate.get("resourceNames"))
    )
    return NormalizedCachePolicy(
        ...
        read=NormalizedCacheRead(
            ...
            stale_seconds=_presence_alias(
                read, ("staleSeconds", "stale"), f"{source_path}.read"
            ),
        ...
        invalidate=NormalizedCacheInvalidation(
            ...
            operations=tuple(
                build_reference(
                    ...
                    owner=owner,
                    source_path=f"{source_path}.invalidate.operations.{index}",
            ...
            resources=tuple(
                build_reference(
                    ...
                    owner=owner,
                    source_path=f"{source_path}.invalidate.resources.{index}",
...
def _transport(raw: Mapping[str, Any]) -> NormalizedRuntimeTransport:
    ...
    return NormalizedRuntimeTransport(
        inbound=NormalizedTransportInbound(
            ...
            user_agent=_presence_alias(
                inbound,
                ("userAgent", "user_agent"),
                "runtime.transport.inbound",
            ),
...
def _hook_uses(
    ...
    *,
    ...
    resource: str | None,
    hook_targets: Mapping[str, NormalizedHookDefinition],
) -> NormalizedRuntimeHooks:
    return NormalizedRuntimeHooks(
        before_handler=_phase_hooks(
            raw.get("beforeHandler", raw.get("before_handler")),
        ...
        after_success=_phase_hooks(
            raw.get("afterSuccess", raw.get("after_success")),
            phase="after_success",
            owner=owner,
            source_path=source_path,
            resource=resource,
        ...
        after_error=_phase_hooks(
            raw.get("afterError", raw.get("after_error")),
            phase="after_error",
            owner=owner,
            source_path=source_path,
            resource=resource,
    ...
    *,
    ...
    for index, item in enumerate(_sequence(value)):
        ...
        reference = build_reference(
            ...
            owner=owner,
            source_path=f"{source_path}.{phase}.{index}",
        ...
        uses.append(
            NormalizedHookUse(
                ...
                source=source_object(
                    raw,
                    source_path=f"{source_path}.{phase}.{index}",
                    known_keys={"ref", "hook", "order"},
                ),
            )
        )
    return tuple(sorted(uses, key=lambda item: (item.order, item.ref)))
...
def _data_source(
    ...
    *,
    ...
    return NormalizedDataSource(
        ...
        item=_schema_use(
            raw.get("item", raw.get("itemSchema")),
            owner=owner,
...
def _target_aliases(values: tuple[Any, ...], kind: ReferenceKind) -> dict[str, Any]:
    targets: dict[str, Any] = {}
    for value in values:
        identifier = str(getattr(value, "id", ""))
        if not identifier:
            continue
        targets[identifier] = value
        targets[f"#/x-codegen/{kind.value}s/{identifier}"] = value
    return targets
