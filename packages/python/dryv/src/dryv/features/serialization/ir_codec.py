...
def contract_to_document(contract: Contract) -> dict[str, object]:
    return {"contract": _encode(contract), "format": _FORMAT, "irVersion": str(IR_API_VERSION)}


def contract_to_json(contract: Contract, *, pretty: bool = True) -> str:
    document = contract_to_document(contract)
    return json.dumps(document, ensure_ascii=False, indent=2 if pretty else None, separators=None if pretty else (",", ":"), sort_keys=True) + ("\n" if pretty else "")


def contract_to_yaml(contract: Contract) -> str:
    return yaml.safe_dump(contract_to_document(contract), allow_unicode=True, default_flow_style=False, sort_keys=True)
...
def validate_transport(document: object) -> Diagnostics:
    try:
        ...
        return Diagnostics((Diagnostic(code=exc.code, severity=DiagnosticSeverity.ERROR, message=exc.message, details=(("path", exc.path),)),))
    return Diagnostics()
...
def _encode(value: object) -> object:
    ...
    if is_dataclass(value):
        ...
        return result
    raise IrCodecError("IR_CODEC_VALUE", f"unsupported IR value {type(value).__name__}")
...
def _decode(value: object, *, path: str, depth: int, state: _DecodeState) -> object:
    ...
    if isinstance(value, list | dict):
        ...
        if identity in state.active:
            raise IrCodecError("IR_CODEC_CYCLE", "recursive IR transport values are not supported", path=path)
...
def _decode_object(value: dict[object, object], *, path: str, depth: int, state: _DecodeState) -> object:
    ...
    if "$enum" in typed:
        if set(typed) != {"$enum", "value"} or not isinstance(typed["$enum"], str):
            raise IrCodecError("IR_CODEC_ENUM", "invalid enum document", path=path)
        enum_type = _enum_registry().get(typed["$enum"])
        if enum_type is None:
            raise IrCodecError("IR_CODEC_ENUM", f"unknown IR enum {typed['$enum']!r}", path=path)
        try:
    ...
    if "$type" in typed:
        ...
        kwargs = {key: _decode(item, path=f"{path}.{key}", depth=depth + 1, state=state) for key, item in typed.items() if key != "$type"}
        try:
...
__all__ = ["IrCodecError", "contract_from_document", "contract_from_json", "contract_from_yaml", "contract_to_document", "contract_to_json", "contract_to_yaml", "validate_transport"]
