...
def resolve_pack(
    ...
    *,
    ...
    _reject_unplanned_execution(manifest)
    ...
    if unknown_options:
        raise PackConfigurationError(
            "PACK_UNKNOWN_OPTION",
            f"unknown pack option {unknown_options[0]!r}",
        )
    ...
    for name in sorted(option_defs):
        ...
        if definition.required and not supplied and definition.default is None:
            raise PackConfigurationError(
                "PACK_REQUIRED_OPTION",
                f"required pack option {name!r} is missing",
            )
        if definition.choices and value not in definition.choices:
            raise PackConfigurationError(
                "PACK_OPTION_CHOICE",
                f"pack option {name!r} is not an allowed value",
            )
    ...
    if unknown_bindings:
        raise PackConfigurationError(
            "PACK_UNKNOWN_BINDING",
            f"unknown pack binding {unknown_bindings[0]!r}",
        )
    missing_bindings = sorted(
        name
        for name, definition in binding_defs.items()
        if definition.required and name not in provided_bindings
    )
    if missing_bindings:
        raise PackConfigurationError(
            "PACK_REQUIRED_BINDING",
            f"required pack binding {missing_bindings[0]!r} is missing",
        )
    ...
    for selection in manifest.selections:
        ...
        if missing:
            raise PackConfigurationError(
                ...
                f"selection {selection.key!r} references unknown binding {missing[0]!r}",
            )
    ...
    for template in manifest.templates:
        if template.selection not in selection_keys:
            raise PackConfigurationError(
                "PACK_MISSING_SELECTION",
                f"template {template.key!r} references missing selection {template.selection!r}",
            )
        missing = sorted(set(template.depends_on) - template_keys)
        if missing:
            raise PackConfigurationError(
                "PACK_MISSING_TEMPLATE",
                f"template {template.key!r} depends on unknown template {missing[0]!r}",
            )
        if template.key in template.depends_on:
            raise PackConfigurationError(
                "PACK_TEMPLATE_CYCLE",
                f"template {template.key!r} cannot depend on itself",
            )

    _validate_template_cycles(manifest)
    return ResolvedPack(
        manifest,
        tuple(resolved_options),
        tuple(sorted(provided_bindings.items())),
        manifest_resource_id,
        manifest_hash,
    )


def _reject_unplanned_execution(manifest: PackManifest) -> None:
    declarations = (
    ...
    for name, value in declarations:
        if value:
            raise PackConfigurationError(
                "PACK_EXECUTION_UNSUPPORTED",
                f"pack {name!r} declarations are not supported until Dryv exposes an explicit planned execution contract",
                path=f"$.{name}",
            )
...
def _validate_template_cycles(manifest: PackManifest) -> None:
    ...
    def visit(key: str) -> None:
        ...
        if key in active:
            ...
            raise PackConfigurationError(
                "PACK_TEMPLATE_CYCLE",
                f"template dependency cycle: {cycle}",
            )
        active.append(key)
        try:
