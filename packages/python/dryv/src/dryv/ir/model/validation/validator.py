...
from ..operations import Operation, OperationOutput
...
class ContractValidator:
    ...
    def _validate_group(self, group: Group, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
    ...
    def _validate_schema(self, schema: Schema, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
    ...
    def _validate_type(self, expression: TypeExpression, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
    ...
    def _validate_schema_use(self, schema_use: SchemaUse, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        self._require(schema_use.schema, index.schemas, "IR_MISSING_SCHEMA", "schema use", owner, diagnostics)

    def _validate_output(self, output: OperationOutput, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        self._require_schema(output.schema, "operation output", owner, index, diagnostics)

    def _validate_effects(self, effects: OperationEffects, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
    ...
    def _validate_operation_facets(self, facets: OperationFacets, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
    ...
    def _validate_access(self, access: AccessFacet | None, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        if access is None:
            return
    ...
    def _validate_trigger(self, trigger: TriggerFacet | None, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        if trigger is None:
            return
        ...
        if trigger.kind is TriggerKind.STORAGE and trigger.storage is not None:
            self._require(trigger.storage, index.storage, "IR_MISSING_STORAGE_MAPPING", "storage trigger", owner, diagnostics)

    def _validate_execution(self, execution: ExecutionFacet | None, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        if execution is None:
            return
        for hook in execution.hooks:
            self._require(hook.operation, index.operations, "IR_MISSING_OPERATION", "execution hook operation", owner, diagnostics)
    ...
    def _validate_view(self, view: View, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        ...
        for trigger in view.triggers:
            self._require(trigger.operation, index.operations, "IR_MISSING_OPERATION", "view trigger operation", view, diagnostics)
    ...
    def _validate_workflow_facets(self, facets: WorkflowFacets, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        self._validate_trigger(facets.trigger, owner, index, diagnostics)
        self._validate_access(facets.access, owner, index, diagnostics)
        self._validate_execution(facets.execution, owner, index, diagnostics)
        if facets.events is not None:
    ...
    def _validate_workflow_step(self, step: WorkflowStep, workflow: Workflow, step_names: set[str], index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
        ...
        if step.compensation is not None:
            self._require(step.compensation.operation, index.operations, "IR_MISSING_OPERATION", "workflow compensation operation", workflow, diagnostics)
        ...
        if step.kind is WorkflowStepKind.DECISION:
            for case in step.decision_cases:
                if case.target not in step_names:
                    diagnostics.append(_error("IR_MISSING_WORKFLOW_STEP", f"workflow decision target {case.target!r} does not exist", workflow, (("step", case.target),)))

    def _require_schema(self, semantic_id: SemanticId | None, relation: str, owner: object, index: SemanticIndex, diagnostics: list[Diagnostic]) -> None:
    ...
    def _require(
        ...
        if semantic_id in collection:
            return
        diagnostics.append(_error(code, f"{relation} references missing semantic id {semantic_id}", owner, (("id", str(semantic_id)), ("relation", relation))))
...
def _error(code: str, message: str, owner: object, details: tuple[tuple[str, object], ...]) -> Diagnostic:
    return Diagnostic(
        code=code,
