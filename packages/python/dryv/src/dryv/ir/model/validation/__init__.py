...
class ContractValidator(_BaseContractValidator):
    def validate(self, contract: Contract) -> Diagnostics:
        ...
        return base.extend(validate_property_contract(contract, index)).extend(validate_schema_extensions(contract, index)).extend(validate_additional_contract(contract, index))
