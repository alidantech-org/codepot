...
def validate_workflow_graphs(contract: Contract, index: SemanticIndex) -> Diagnostics:
    ...
    for group in walk_groups(contract.groups):
        for workflow in group.workflows:
            ...
            graph[workflow.id] = tuple(step.workflow for step in walk_workflow_steps(workflow.steps) if step.workflow is not None)
    ...
    def visit(workflow_id: SemanticId) -> None:
        ...
        if workflow_id in visiting:
            ...
            diagnostics.append(Diagnostic(code="IR_WORKFLOW_CYCLE", severity=DiagnosticSeverity.ERROR, message="child workflow cycle: " + " -> ".join(str(item) for item in cycle), span=owner_span(owner) if owner is not None else None))
            return
