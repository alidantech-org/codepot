...
- dependency-free `dryv` package metadata;
...
OpenAPI, TypeScript target, Dart target, and Jinja package-local foundation work may proceed only through the published public namespaces and must not import `dryv.domain` implementation modules.
...
- Output hashes/state remain in ownership/generation state, not `dryv.lock.yaml`.
- The future `dryv` namespace cutover must occur in an isolated release environment.

## Next action

Run from `packages/python/dryv`:
