...
This plan implements only the approved v2 architecture and closed semantic kernel. The old `packages/python/dryv` package remains the old runtime; v2 does not add old configuration decoders, old execution paths, or imports from it.
...
- [x] **DOC-003** Document complete `dryv.yaml` and `DryvPack.yaml` schemas.
...
- [ ] **CACHE-002** Store output digests and incremental state outside `dryv.lock.yaml`.
...
- [ ] **OA-001+** Complete the aligned `dryv-openapi` ledger, including typed `x-codegen` mapping into the closed kernel.
...
- [ ] **LOCK-001** Implement `dryv.lock.yaml` with requested refs, resolved commits, digests, plugin versions, and behavior versions.
