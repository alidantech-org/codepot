...
Dryv must integrate with real Node, Dart, Flutter, and future project ecosystems without turning the semantic kernel or pack manifest into a package-manager abstraction language.
...
- no pack-level `dependencies`, `integration`, traits, setup profiles, or automatic package-manager conversion exists in the baseline `DryvPack.yaml`.
...
It does not add arbitrary manifest contribution dictionaries to `DryvPack.yaml`.
...
One `DryvPack.yaml` represents one coherent product and deterministic file inventory.
