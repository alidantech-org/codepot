...
The Python API is the primary Dryv interface. CLI, MCP, HTTP, playground, notebook, IDE, and future blast-radius UI integrations call the same application services.
...
```python
from dryv import Dryv

runtime = Dryv.standard()
```
...
result = runtime.generate(
    GenerationRequest.from_project_file("dryv.yaml")
)
```
