...
|---|---|---:|---:|---|---|
| #28 | `dryv-template-jinja` | strong | high | review | Close exact Ruff/real-core/real-wheel gates; address two low compatibility/diagnostic items. |
| #29 | `dryv-openapi` | directionally correct | foundation only | not usable | Critical repair: add missing adapter/facade, session isolation, YAML hardening, public conformance/distribution tests. |
| #30 | `dryv-language-typescript` | strong | high | review | Fix direct option validation and reproduce exact release/wheel gates. |
| #30 | `dryv-language-dart` | strong | high | review | Fix direct option validation, run real Dart SDK oracle, and reproduce exact release/wheel gates. |
...
```text
packages/python/dryv-template-jinja/docs/audits/2026-07-27-pr-28-audit.md
packages/python/dryv-template-jinja/docs/tasks/AUDIT_FIXES.md
```
...
However, the package is not a functioning source adapter. The registered factory imports a missing `dryv_openapi.adapter` module. There is no final composed `normalize()` pipeline or public source-adapter conformance path.
...
```text
packages/python/dryv-openapi/docs/audits/2026-07-27-pr-29-audit.md
