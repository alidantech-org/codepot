from dryv.ir import Contract, Event, Failure, Group, GuidanceKind, GuidanceNote, KernelData, Name, Operation, Policy, Presentation, PresentationChannel, PresentationEntry, SemanticId, TagSet, View, Workflow, WorkflowStep, WorkflowStepKind, validate_contract
...
def test_workflow_can_compose_child_workflow_and_cycles_are_rejected() -> None:
    ...
    parent = Workflow(SemanticId("workflow.parent"), Name("Parent"), steps=(WorkflowStep("child", WorkflowStepKind.WORKFLOW, workflow=child.id),))
    group = Group(SemanticId("group.test"), Name("test"), workflows=(parent, child))
    assert not validate_contract(Contract(SemanticId("contract.test"), Name("Test"), (group,))).has_errors

    first = Workflow(SemanticId("workflow.first"), Name("First"), steps=(WorkflowStep("second", WorkflowStepKind.WORKFLOW, workflow=SemanticId("workflow.second")),))
    second = Workflow(SemanticId("workflow.second"), Name("Second"), steps=(WorkflowStep("first", WorkflowStepKind.WORKFLOW, workflow=first.id),))
    cycle = Contract(SemanticId("contract.cycle"), Name("Cycle"), (Group(SemanticId("group.cycle"), Name("cycle"), workflows=(first, second)),))
    assert any(item.code == "IR_WORKFLOW_CYCLE" for item in validate_contract(cycle).errors)
...
def test_presentation_can_reference_related_behavior() -> None:
    ...
    entry = PresentationEntry(SemanticId("entry.test"), Name("Entry"), view.id, policies=(policy.id,), operations=(operation.id,), events=(event.id,), workflows=(workflow.id,))
    presentation = Presentation(SemanticId("presentation.test"), Name("Presentation"), PresentationChannel.WEB, entries=(entry,))
    group = Group(SemanticId("group.test"), Name("test"), views=(view,), policies=(policy,), operations=(operation,), events=(event,), workflows=(workflow,))
    assert not validate_contract(Contract(SemanticId("contract.test"), Name("Test"), (group,), presentations=(presentation,))).has_errors
