...
from .core import Diagnostic, DiagnosticSeverity, Diagnostics, RelatedLocation, SourceSpan
from .ir import (
    AccessFacet,
    Contract,
    Event,
...
class ContractValidator:
    def validate(self, contract: Contract) -> Diagnostics:
        ...
        self._register(
            ...
            contract.id,
            "contract",
            _span(contract.data.provenance),
            diagnostics,
        )
        ...
        return Diagnostics(tuple(diagnostics)).sorted()
    ...
    def _register_group(
        ...
        self._register(seen, group.id, "group", _span(group.data.provenance), diagnostics)

        for schema in group.schemas:
            self._register(seen, schema.id, "schema", _span(schema.data.provenance), diagnostics)
            ...
            for schema_field in schema.fields:
                self._register(
                    ...
                    schema_field.id,
                    "schema_field",
                    _span(schema_field.data.provenance),
                    diagnostics,
                )
        ...
        for operation in group.operations:
            self._register(
                ...
                operation.id,
                "operation",
                _span(operation.data.provenance),
                diagnostics,
            )
            index.operations.setdefault(operation.id, operation)

        for view in _walk_views(group.views):
            self._register(seen, view.id, "view", _span(view.data.provenance), diagnostics)
        ...
        for workflow in group.workflows:
            self._register(
                ...
                workflow.id,
                "workflow",
                _span(workflow.data.provenance),
                diagnostics,
            )
        ...
        for policy in group.policies:
            self._register(
                ...
                policy.id,
                "policy",
                _span(policy.data.provenance),
                diagnostics,
            )
            index.policies.add(policy.id)

        for event in group.events:
            self._register(seen, event.id, "event", _span(event.data.provenance), diagnostics)
    ...
    def _register(
        ...
        if previous is None:
            seen[semantic_id] = _Seen(kind=kind, span=span)
            return
        related = ()
        if previous.span is not None:
            related = (RelatedLocation(f"first declared as {previous.kind}", previous.span),)
        diagnostics.append(
            Diagnostic(
    ...
    def _validate_workflow(
        ...
        all_steps = _walk_steps(workflow.steps)
        ...
        for transition in workflow.transitions:
            if transition.source not in step_names:
                diagnostics.append(
                    _error(
                        "IR_MISSING_WORKFLOW_STEP",
                        f"workflow transition source {transition.source!r} does not exist",
                        workflow,
                        (("step", transition.source),),
                    )
                )
            if transition.target not in step_names:
                diagnostics.append(
                    _error(
                        "IR_MISSING_WORKFLOW_STEP",
                        f"workflow transition target {transition.target!r} does not exist",
                        workflow,
...
def _walk_views(views: tuple[View, ...]) -> tuple[View, ...]:
    result: list[View] = []
...
def _walk_steps(steps: tuple[WorkflowStep, ...]) -> tuple[WorkflowStep, ...]:
...
def _span(provenance: object) -> SourceSpan | None:
    return getattr(provenance, "span", None) if provenance is not None else None


def _owner_span(owner: object) -> SourceSpan | None:
    data = getattr(owner, "data", None)
    provenance = getattr(data, "provenance", None)
    return _span(provenance)
