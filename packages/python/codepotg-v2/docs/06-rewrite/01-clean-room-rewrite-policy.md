...
The v2 implementation is a new architecture, not a refactor of the existing `packages/python/codepotg` internals.
...
- a decoder for old `codepotg.yaml` `tasks`;
...
- old pack paths/emissions/folders are redesigned as `CodepotgPack.yaml` content, selections, patterns, files, bindings, and profiles.
