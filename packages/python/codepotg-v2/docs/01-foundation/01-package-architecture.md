...
CodepotG v2 is a clean, importable Python application with explicit domain boundaries and independently installable adapters/infrastructure packages.

The implementation directory is `packages/python/codepotg-v2`; the eventual supported Python namespace is `codepotg`.
...
`codepotg-core` is minimal. `codepotg` is the batteries-included distribution installing compatible official defaults.

## Namespace transition rule

V2 is developed/tested separately from the old package. It must not rely on side-by-side installation of two distributions owning `codepotg`.
