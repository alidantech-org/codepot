...
The `codepotg` distribution depends on compatible releases of:
...
```bash
pip install "codepotg[kotlin]"
pip install acme-codepotg-language-csharp
```
...
The new distribution is built/tested in isolated environments while the old package remains in the repository. The final release replaces the old published runtime; the two distributions are not expected to be installed side by side while both own the `codepotg` import namespace.
