...
- [`02-git-github-locking-and-trust.md`](02-git-github-locking-and-trust.md) — direct `source.local`/`source.git`, authentication, immutable resolution, `codepotg.lock.yaml`, cache, private packs, approvals, and future discovery.
...
- `codepotg-core` — minimal application/domain/plugin contracts;
- `codepotg` — normal installation with compatible OpenAPI, TypeScript, Dart, Jinja, and initial SDK packs.
...
Branches and tags resolve to immutable commits stored with pack subdirectory and content/manifest digests in `codepotg.lock.yaml`.

A future site may help users discover packs, but its output is a complete source block inserted into `codepotg.yaml`; runtime resolution stays self-contained.
