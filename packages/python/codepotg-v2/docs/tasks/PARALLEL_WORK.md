...
| TS-001..TS-010, DART-001..DART-010 | coordinated TypeScript and Dart target-language adapters | language-adapter audit-fix lanes | review | `packages/python/codepotg-language-typescript/**`; `packages/python/codepotg-language-dart/**`; coordination updates in this file only | current public `TargetAdapter` protocol | PR #30 is architecturally correct and both packages remain separate. Required repair: direct options constructors accept raw non-enum values that can silently bypass identity-based policies. Release remains blocked on exact synchronized Ruff/core/build/real-wheel checks; Dart also requires a real SDK oracle. TS-006/DART-006 and TS-009/DART-009 remain blocked by missing planner/module/official-pack contracts. |
| AUTHOR-001..AUTHOR-013, AUTHOR-015..AUTHOR-020, AUTHOR-022, AUTHOR-024, AUTHOR-027..AUTHOR-030 | `codepotg-author` typed Python authoring compiler and canonical IR transport | GPT-5.6 Thinking / CODEPOT session 2026-07-27 (`chatgpt/codepotx-restart-codepotg-author`) | in_progress | `packages/python/codepotg-author/**`; coordination updates in this file only | Public `codepotg.ir`, `codepotg.diagnostics`, naming, versions, and validation facades synchronized through base `4e310343` | AUTHOR-014, AUTHOR-021, AUTHOR-023, AUTHOR-025, AUTHOR-026, extended AUTHOR-019, and shared canonical-codec ownership in AUTHOR-028 remain blocked on public core contracts. Package-local transport will use only public immutable IR APIs and record the ownership blocker. |
...
| `codepotg-language-typescript` | strict option repair and release verification; later planner/pack integration when contracts exist | `codepotg.plugins`, `codepotg.ports.TargetAdapter`, target/path request and result types, target conformance helper |
| `codepotg-language-dart` | strict option repair, Dart SDK oracle, and release verification; later planner/pack integration when contracts exist | same public target contracts as TypeScript |
