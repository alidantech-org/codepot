...
| TS-001..TS-010, DART-001..DART-010 | coordinated TypeScript and Dart target-language adapters | current ChatGPT implementation session | claimed | `packages/python/codepotg-language-typescript/**`; `packages/python/codepotg-language-dart/**`; coordination updates in this file only | current public CodepotG `TargetAdapter` protocol | Branch: `chatgpt/codepotx-restart-language-adapters`. Both adapters are implemented by one agent but remain separate independently installable distributions. Neither package may import the other package's internals, and no shared runtime package will be introduced. Cross-package integration tasks may remain blocked when Jinja, packs, or missing core planning contracts are required. No core implementation file will be modified. |
