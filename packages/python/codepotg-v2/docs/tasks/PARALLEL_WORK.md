...
| TS-001..TS-010, DART-001..DART-010 | coordinated TypeScript and Dart target-language adapters | current ChatGPT implementation session | review | `packages/python/codepotg-language-typescript/**`; `packages/python/codepotg-language-dart/**`; coordination updates in this file only | current public CodepotG `TargetAdapter` protocol | Branch: `chatgpt/codepotx-restart-language-adapters`. Both complete package suites pass in the local public-contract harness, so the coordinated claim is ready for review but not completion. Both adapters are implemented by one agent and remain separate independently installable distributions. Neither package imports the other package's internals, and no shared runtime package was introduced. TS-009/DART-009 remain blocked on official Jinja/pack/planning contracts; final completion also remains blocked on the exact core, Ruff, build, and real-wheel verification commands. No core implementation file was modified. |
