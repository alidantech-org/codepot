# CodepotG v2 orchestrator implementation plan
...
This plan does not restore the old CodepotG runtime or make commands, Git, cache, or incremental generation implicit.
...
- verify the reorganized `codepotg-core` package against the synchronized checkout;
...
**Verification still required:** focused property tests, complete core suite, JSON/YAML round trips, selector ordering, and compatibility review with `codepotg-author`.
...
- `requires.codepotg` and `requires.ir` specifiers;
