...
CodepotG v2 must understand configuration through typed models and registered decoders. Raw YAML mappings must never flow into application, domain, adapter, or template logic.
...
```text
(Project, codepotg.dev/v2)
(TemplatePack, codepotg.dev/v2)
(LockFile, codepotg.dev/v1)
```
...
- support update-in-place for `codepotg configure` without rewriting unrelated pack entries where practical.
