...
class FakeTargetAdapter:
    ...
    def resolve_module_path(self, request: ModulePathRequest) -> ModulePathFacts:
        ...
        current = PurePosixPath(request.current_artifact).parent
        provider = PurePosixPath(request.provider_artifact)
        relative = PurePosixPath("..", provider.name) if current != provider.parent else PurePosixPath(provider.name)
...
def test_adapter_protocols_and_public_conformance_helpers(connected_contract: Contract) -> None:
