...
def test_distribution_metadata_typing_and_entry_point() -> None:
    package = distribution("codepotg-language-typescript")
    assert package.version == "0.1.0a1"
    assert (Path(codepotg_language_typescript.__file__).parent / "py.typed").is_file()
    entry = next(
        item
...
def test_built_wheel_contents(
    ...
    assert any(name == "codepotg_language_typescript/py.typed" for name in names)
    assert any(name.endswith(".dist-info/entry_points.txt") for name in names)
    assert not any(name.startswith(("tests/", "benchmarks/")) for name in names)
    assert not any("codepotg_language_dart" in name for name in names)
