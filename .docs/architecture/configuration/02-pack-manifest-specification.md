# `DryvPack.yaml` pack specification

## Purpose

`DryvPack.yaml` registers only pack behavior that cannot be inferred safely from the pack filesystem.
...
Required typed schema version. Initial value: `dryv.dev/v1`.
...
Compatibility requirements for Dryv, the semantic-kernel/IR behavior, and later stable adapter contracts. Compatibility is checked before planning.
...
- Dryv resolves destination-relative and target-aware module/path facts before rendering;
...
Dryv does not parse rendered code to guess exports. Symbols are used by dependency matching, barrels, conflict validation, plan inspection, and impact reporting.
