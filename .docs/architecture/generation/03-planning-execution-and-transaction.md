...
Dryv compiles a complete immutable generation plan before any template renders or destination file changes.
...
Before artifact planning, Dryv validates kernel semantics including:
...
A semantic identity remains stable across renames only when the source supplies or Dryv can derive a stable identity independently of the display name. Dryv must not guess that delete-plus-add is a rename.
...
Templates, macros, partials, and static files own every emitted character. Dryv provides facts, relationships, names, symbols, paths, and ordering; it does not render types, literals, validators, decorators, comments, framework logic, workflow logic, or import/export statements on behalf of packs.
...
If exact impact cannot be proven, Dryv regenerates a broader safe scope. Incremental execution must never sacrifice deterministic equivalence with a complete generation.
...
Generated output hashes do not belong in `dryv.lock.yaml`. The lock records resolved inputs and behavior identity; the ownership/generation-state manifest records outputs.

## Lifecycle

Supported writer intent includes:

- managed: Dryv may replace or remove owned output;
