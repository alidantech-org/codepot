...
These rules allow several agents to implement Dryv without changing the architecture or duplicating work.
...
- copy modules from `packages/python/dryv` into v2 or import old internals;
...
- store generated output hashes in `dryv.lock.yaml`;
